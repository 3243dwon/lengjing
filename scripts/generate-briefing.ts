...

{
  ...
  "marketImpactAnalysis": {
    "summary": "一句话总结今日对A股和港股的整体影响（30字以内）",
    "items": [
      {
        "id": 1,
        "event": "关键事件描述（20字以内）",
        "chinaImpact": "对A股的具体影响分析（60-100字），包含可能受影响的具体板块或个股",
        "hkImpact": "对港股的具体影响分析（60-100字），包含可能受影响的具体板块或个股（如科技股、地产股、金融股等）",
...
7. **市场影响分析 (marketImpactAnalysis)**：针对最影响中国A股和港股的3个关键事件（从今日要点、海外视角、宏观脉搏中挑选），分别分析对两个市场的具体影响。要求：
   - 针对具体板块和类型（科技、金融、地产、消费、新能源、医药、出口等）
   - 如果事件对某一市场影响微弱，也要说明原因（如「对A股影响有限，因内需导向」）
   - A股侧重政策、内需、央企；港股侧重中概、科技、地产、汇率
   - affectedSectors 列出2-4个具体受影响板块
...
- 输入数据中可能包含 "chinaNews" 字段（来自新浪财经的中文新闻）。优先使用这些中国本地新闻来丰富「今日要点」和「行业聚焦」。这些新闻已经是中文，不需要翻译。
- 如果某个数据源为空或缺失，跳过即可，不要编造数据来填充。用已有数据生成尽可能完整的简报。`;
